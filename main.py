...
if IS_WEB:
    ...
    LogFont = pygame.font.SysFont("monospace", 15)
    NEOfont = pygame.font.SysFont("sans", 10)
else:
...
Tracking_text = font.render("CURRENTLY TRACKING -- NEOS", True, (0, 150, 200))
...
async def fetch_async_url(url_string):
    try:
        import platform
        response = await platform.window.fetch(url_string)
        text_data = await response.text()
        return json.loads(text_data)
    except Exception:
        return None

async def fetch_real_orbital_elements_async(asteroid_id):
    clean_id = asteroid_id.replace("(", "").replace(")", "").replace(" ", "%20").strip()
    sbdb_url = f"https://ssd-api.jpl.nasa.gov/sbdb.api?sstr={clean_id}&phys-par=0"
    data = await fetch_async_url(sbdb_url)
    if data and "orbit" in data and "elements" in data["orbit"]:
...
async def main():
    global angle, selected_Asteroid, time_speed, orbitline_Toggle, quickInfotoggle, Tracking_text
    global last_button_press_time, radar_Integ, optical_Integ, comms_Integ, last_update_time, update_delay

    feed_url = f"https://api.nasa.gov/neo/rest/v1/feed?start_date=2026-01-01&end_date=2026-01-08&api_key={API_KEY}"
    
    console_logs.append("[SYSTEM] ATTEMPTING LIVE NASA DATAFEED SYNC...")
    
    data = None
    if IS_WEB:
        data = await fetch_async_url(feed_url)
    else:
        try:
    ...
    if data and "near_earth_objects" in data:
        console_logs.append("[SUCCESS] LIVE SENSOR DATA SYNCHRONIZED")
        ...
        for day in asteroids:
            for asteroid in asteroids[day]:
                ...
                orb_elements = None
                if IS_WEB:
                    orb_elements = await fetch_real_orbital_elements_async(asteroid_id)
                else:
                    try:
                        sbdb_url = f"https://ssd-api.jpl.nasa.gov/sbdb.api?sstr={asteroid_id.replace('(', '').replace(')', '').replace(' ', '%20').strip()}&phys-par=0"
                        with urllib.request.urlopen(sbdb_url, timeout=3) as resp:
                            sbdb_data = json.loads(resp.read().decode())
                            if sbdb_data and "orbit" in sbdb_data and "elements" in sbdb_data["orbit"]:
                                elements = sbdb_data["orbit"]["elements"]
                                elem_dict = {item["name"]: float(item["value"]) for item in elements}
                                orb_elements = (elem_dict.get("a"), elem_dict.get("e"), elem_dict.get("i"), elem_dict.get("ma"))
                    except Exception:
                        orb_elements = None
        ...
        console_logs.append("[WARN] API DATA UNAVAILABLE // USING MOCK DATA")
        Tracking_text = font.render("SAVED DATA VERSION", True, (0, 150, 200))
        
        for i in range(40):
            mock_dist = random.uniform(0.05, 1.5)
            mock_vel = random.uniform(10.0, 35.0)
            base_a = 1.2 + (mock_dist * 0.5)
            
            asteroidsPoints.append({
                "name": f"NEO-SIM-{random.randint(100000, 999999)}", 
                "size": random.randint(1, 7), 
                "approach": {},
                "a": 140 + (mock_dist * 180), 
                "e": random.uniform(0.05, 0.35), 
                "M": random.uniform(0, math.pi * 2), 
                "n": 0.002 / math.sqrt(max(0.1, base_a**3)),
                "tilt": random.uniform(-6, 12), 
                "z": random.uniform(-1, 1), 
                "velocity": mock_vel, 
                "norm_vel": (mock_vel - 5) / 30, 
                "danger": "HIGH" if mock_vel > 25 else "LOW"
            })
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if btn_slow.collidepoint(mouse_pos):
                    ...
                    time_speed = 0
                    console_logs.append("[SYSTEMS] SYSTEM PAUSED")
                elif btn_play.collidepoint(mouse_pos):
                    time_speed = 1
                    console_logs.append("[SYSTEMS] SYSTEM RESUMED")
                elif btn_info.collidepoint(mouse_pos):
                    quickInfotoggle = not quickInfotoggle
                    console_logs.append("[INFO] QUICK INFO TOGGLED")
                    ...
                    try:
                        if os.path.exists(target_dir):
                            ...
                            if reports:
                                ...
                                console_logs.append("[ERROR] NO TELEMETRY REPORTS SAVED")
                        else:
